...
export default function ForgottenPassword() {
  ...
  // ForgottenPassword
  async function handleForgottenPassword(initialValues) {
  ...
  // Formik
  let forgottenPasswordFormik = useFormik({
    initialValues: {
      email: "",
    },
    validationSchema,
  ...
  return (
    <>
      <h2 className="uppercase text-center font-bold text-2xl text-green-700 mb-5">
        Please enter your Email
      </h2>
      <div className=" py-10 flex w-full justify-center items-center relative custom-height">
        <form
          onSubmit={forgottenPasswordFormik.handleSubmit}
        ...
        >
          <div className="mb-1">
            ...
            <input
              ...
              id="email"
              name="email"
              onBlur={forgottenPasswordFormik.handleBlur}
              onChange={forgottenPasswordFormik.handleChange}
              value={forgottenPasswordFormik.values.email}
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-green-500 dark:focus:border-green-500"
              placeholder="xxx@yyy.com"
            />
          </div>
          {forgottenPasswordFormik.touched.email &&
          forgottenPasswordFormik.errors.email ? (
            <div
              className="p-2 mb-1 text-sm text-red-50 rounded-lg bg-red-500 dark:bg-gray-800 dark:text-red-400"
              role="alert"
            >
              <span className="font-medium">
                Danger alert! {forgottenPasswordFormik.errors.email}
              </span>
            </div>
          ) : null}
          <div className="flex justify-between items-center">
            <div className="w-1/2">
              <button
                type="submit"
                disabled={
                  forgottenPasswordFormik.isValid == false ||
                  forgottenPasswordFormik.dirty == false ||
                  isLoading
                }
